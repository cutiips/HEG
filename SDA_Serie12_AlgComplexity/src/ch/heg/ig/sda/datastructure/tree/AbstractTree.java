...
public abstract class AbstractTree<E> implements Tree<E> {
     ...
     * @return
     */
    public int heightBad(Position<E> position) {
        int height = 0; // height=0 si position est externe (aucun enfant)

        for (Position<E> p : positions()) { // O(n)
            if (isExternal(p))
                height = Math.max(height, depth(p)); // pire cas O(n2) lorsque l'arbre la profondeur de l'arbre est égale au nombre d'éléments n
        }
    ...
    public Collection<Position<E>> positions(){
        return preOrder();
    }
